...
import { Class, Engine, Entry, Namespace, Method, Return, String } from "php-parser";
...
export const getRules = ({ formRequestPath }: GetRulesType) => {
	...
	const namespace = ast.children.find((child) => child.kind === 'namespace') as Namespace
	const classBlock = namespace.children.find((child) => child.kind === 'class') as Class

	const rulesMethod = classBlock.body.find((node) => node.kind === 'method' && !(typeof node.name == 'string') && node.name.name === 'rules') as Method|undefined

	const returnBlock = rulesMethod?.body?.children.find((child) => child.kind === 'return') as Return | null
